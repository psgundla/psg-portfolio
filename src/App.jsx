...
import { LuPlay, LuPause, LuSun, LuMoon, LuCoffee, LuCodeXml, LuSearch } from "react-icons/lu";
    ...
    <div className="site-shell">
      ...
      <main id="main">
        <section className="hero editorial-hero" ref={hero} aria-labelledby="hero-title">
          ...
          <motion.div className="hero-copy" initial={{ opacity: 0, y: reduced ? 0 : 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduced ? 0 : 0.65 }}>
            <p className="hero-summary">Connecting glioma histology with spatial biology through deep learning.</p>
          </motion.div>
        ...
        <div className="research-pair">
          <section className="research-placeholder" aria-label="Reserved section">
            <span className="chapter-index">01 / Coming next</span>
          </section>
        ...
        <motion.section className="section research-record" aria-label="Publications and methods" {...reveal}>
          <ResearchRecord />
          <section id="technical-stack" className="technical-stack" aria-labelledby="technical-stack-title">
          <header>
        ...
        <section id="life" className="section life" aria-labelledby="life-title">
          <div className="life-heading"><div><h2 id="life-title">Life, <em>in frames.</em></h2></div></div>
          ...
          <motion.article className="bunker-card" whileHover={reduced ? {} : { y: -5 }}>
            ...
            <span className="eyebrow">Side project</span><h3>BUNKER</h3><p>Browser-based techno sequencer built with Web Audio.</p><a className="text-link" href="/bunker/index.html" target="_blank" rel="noreferrer">Open sequencer</a>
          </motion.article>
        </section>
        <SocialPosts />
        ...
        <section id="contact" className="contact" aria-labelledby="contact-title">
          ...
          <img className="contact-logo" src={`/brand/signature-${theme === "dark" ? "white" : "black"}.svg`} width="690" height="552" alt="Pranav Swaroop logo" />
          <p className="eyebrow">Have a question or a project in mind?</p>
          ...
          <aside className="contact-profile-card music-card" aria-labelledby="music-title">
            ...
            <div ref={vinyl} className={`vinyl-deck${musicPlaying ? " is-playing" : ""}`} role="img" aria-label="Vintage vinyl record with a red center label and silver tonearm">
              <div className="vinyl-record"><div className="vinyl-label"><img src={`/brand/signature-${theme === "dark" ? "black" : "white"}.svg`} width="690" height="552" alt="" /></div></div>
      ...
      <dialog className="project-dialog" ref={dialog} aria-labelledby="project-dialog-title" onClose={() => setSelectedProject(null)}
        ...
        {selectedProject && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduced ? 0 : 0.2 }}>
          ...
          <div className="project-links">{selectedProject.links.map(link => <a className="text-link" href={link.href} key={link.href} target={link.href.startsWith("mailto:") || link.href.startsWith("#") ? undefined : "_blank"} rel="noreferrer">{link.label}</a>)}</div>
        </motion.div>}
      </dialog>
    </div>
  );
